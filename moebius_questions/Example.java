...
public class Example {
    public List<Integer> removeBigNumbers(List<Integer> data) {
		...
		// returns a new list that is composed of only integers less than or equals to 10
		return resultList;
	}
...
// cause unexpected behavior, as we wouldn't be iterating over the original list, but a modified version of it in every new iteration. 
//
// The best fix is to keep track of any numbers that are less than or equals to 10, and add them to a seperate new list, and return that list as whole. This would prevent
// the ConcurrentModificationException from happening altogether and implement the original solution that was intended. 

// The commented out code below should be the correct revision needed to be made to fix the issue addressed above. 
