...
def largestAltitude(gain):
    # the highest altitude in the start is altitude 0
    highest_altitude = 0
    # Keep track of our current altitude as we accumulate each gain
    current_altitude = 0
    for i in range(len(gain)):
        current_altitude = current_altitude + gain[i]
        # Update the highest altitude if we've reached a new maximum
        highest_altitude = max(highest_altitude, current_altitude)
    return highest_altitude

gain = [-5,1,5,0,-7]
print("Gains: ", gain)
print("Highest Altitude: ", largestAltitude(gain), "\n") 
